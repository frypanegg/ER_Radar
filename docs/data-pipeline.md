...
scripts/collect-news.mjs는 공개 추적 법인(2026-08-25 기준 26곳)의 당해 연도 임금협상·단체협상 관련
...
수집 → 원문 확인 → 사실 반영 → 회귀 검증 → 커밋 → 배포를 한 작업에서 처리한다.
회귀 검증(`npm test`)은 빌드, `tsc --noEmit` 타입 검사, 테스트 53개를 차례로 돌린다.
타입 검사가 여기 들어간 이유는, 2026-08-16에 타입 오류 하나가 빌드·테스트·린트를
모두 통과해 main까지 올라갔기 때문이다. 셋 중 어느 것도 타입을 보지 않았다.
`workflow_dispatch`로 수동 실행할 수 있으며 `force` 입력을 켜면 같은 KST 날짜라도
다시 수집하고 다시 배포한다.

### 원청 직영·하청 판정 규칙 (2026-08-11 개정)

판정은 두 축을 분리한다.

- **배제**는 어휘 신호로 한다. `하청·사내협력사·협력업체·용역·파견·수급업체·계열사 노조`는
  한국 노동 보도에서 표현이 거의 고정이라 정밀도가 높다. 이 축은 규칙이 잘 맞는다.
- **포함**은 두 등급으로 나눈다. 제목에 등록된 교섭단위 정식명칭(예: `금속노조 현대자동차지부`)이
  있으면 `CONFIGURED_UNION_ALIAS`, 법인명 근처(20자 이내)에 `노조·노동조합·지부·노사`
  표현만 있고 배제 신호가 하나도 없으면 `COMPANY_UNION_PHRASE_NO_EXCLUSION_SIGNAL`이다.
  후자는 근거가 약하므로 감사에 등급을 남기고, 원문 URL 확인·단일 법인·단계 규칙 게이트를
  그대로 통과해야만 공개 단계가 바뀐다.

개정 전에는 정식명칭만 근거로 인정해서, 실제로는 원청 직영 교섭 기사인데 전체의 79%가
검토 대기로 빠졌다(`현대제철, 임단협 최종 타결`, `포스코 노사, 단체교섭 조정기간 연장` 등).
즉 규칙이 나누던 축은 직영 대 하청이 아니라 "정식명칭이 제목에 있나 없나"였다.

혼재(`MIXED_NEEDS_SPLIT`)는 **강한 근거와 배제 신호가 함께 있을 때만** 쓴다. 약칭 근거와
하청 신호가 겹치는 기사(`한화오션 하청 노조 파업권 확보`)는 혼재가 아니라 하청 사건이다.

레지스트리에 하청 교섭단위를 추가하면 약한 경로는 그 법인에서 자동으로 꺼진다.

**2026-09-15 추가 (규칙 1.2.0)**: 노조·노사라는 말 없이 법인의 교섭 자체를 주어로 쓴 제목
(`HD현대중공업 임단협 결렬`, `현대차 임금협상 잠정합의`)도 같은 약한 근거로 본다. 등급은
`COMPANY_BARGAINING_TERM_NO_EXCLUSION_SIGNAL`로 따로 남긴다. 법인의 임단협·임금협상은 그
법인 직접고용 근로자의 노조와 맺는 것이기 때문이다. 단, 별칭 뒤가 단어 경계(공백·문장부호·
조사 한 글자)여야 한다. `카카오뱅크 임단협`, `현대차그룹 첫 임단협`은 다른 법인이다.
2026-09-15 배치 기준 직영 판정은 343건에서 429건으로 늘었고 하청 격리는 17건 그대로다.

### 근거 등급과 반영 게이트

- 근거 등급(`sourceTier`)은 실제 검증 상태를 따른다. NAVER가 직접 준 원문이거나 Google News
  링크를 발행사 주소로 되돌린 뒤 도달·제목 일치까지 확인하면 `B`, 그 외에는 `C`다. 이전에는
  상수로 박혀 있어 원문까지 확인한 기사도 미확인 기사와 같은 등급으로 기록됐다.
- 협약유형(임금협상·통합 임단협)을 제목에서 못 뽑는 것은 **단계 갱신을 막지 않는다.** 협약유형은
  화면 필터용 메타데이터이고, 시드에는 이미 사람이 검증한 값이 있으며 반영 단계가 그 값을
  유지한다. 이 항목이 직영으로 판정된 기사의 최대 차단 사유였다.
- 단계는 **뒤로 되돌아가지 않는다.** 파업 기사는 "노사 교섭 계속" 표현을 함께 담아 교착(S4)
  기록을 본교섭(S3)으로 내릴 수 있다. 예외 전이(부결·재교섭 결렬)나 명시적 교착·조정 근거로
  표시된 경우에만 하강을 허용한다.
- **조합원 가결은 최종 체결 근거가 아니다.** `임단협 가결`을 조인·체결과 같은 신호로 묶어 두어
  S7로 올라가던 문제를 고쳤다. 가결·인준은 S6이고, S7은 서명·조인·발효가 확인된 경우다.
- **파업 찬반투표는 인준 투표가 아니다.** 제목 뒤쪽의 "임단협" 때문에 S6 패턴에 걸려 파업으로
  가는 상황이 타결 직전으로 표시되던 문제를 고쳤다.

#### 2026-09-15 개정: 14일 무반영의 원인

2026-09-01~14 동안 공개 기록이 사실상 갱신되지 않았다. 수집은 매일 성공했고 현대차 조인식,
포스코 창사 첫 파업·교섭 재개, HD현대중공업 결렬·부분파업, SK하이닉스 수정 잠정합의 기사가
모두 후보에 있었다. 규칙이 보수적이어서가 아니라 아래 다섯 경로가 구조적으로 막고 있었다.

| 막힌 경로 | 고친 방식 |
| --- | --- |
| 같은 기사가 다른 법인 검색에도 걸리면 URL 중복 제거가 검색 법인을 합쳐 `복수 법인`이 됐다 | 제목에 법인이 있으면 검색어 맥락 법인은 붙이지 않는다 |
| 복수노조 10개 법인은 제목에 정식 노조명이 없으면 `교섭단위 불명`으로 영구 검토 대기 | 공개 기록이 추적하는 테이블(`publicRecordUnitIds`)로 잇는다. 사무직·조종사 등 다른 노조를 가리키는 말이 있으면 잇지 않고, 다른 단위로 확인된 기사(대한항공 조종사노조)는 `different_bargaining_unit`으로 막는다 |
| 교착 사유·인준안 변화·협약유형 주기 충돌 주석의 검토 표시가 단계 반영까지 막았다. 감사 로그 44회분에서 S4로 자동 반영된 기록이 0건이다 | 주석 검토 표시는 남기고, 반영 게이트에서는 뺀다. 반영기는 이 주석을 시드에 쓰지 않는다 |
| `임금교섭 조인식`, `임단협 결렬`, `사후조정`이 단계 신호에 없어 조인식이 S3로 읽혔다 | 체결·교착 신호와 조인 예정 가드를 추가했다 |
| 반영기가 법인별 가장 최근 기사 하나만 게이트에 넣었다. 그 기사가 막히면 앞선 유효 기사도 버렸고, 결렬 뒤에 나온 증권 기사 "임금협상 난항에 노조 부분파업"이 본교섭으로 결렬을 덮을 수도 있었다 | 통과한 기사를 날짜순으로 차례로 적용한다. 각 기사는 앞 기사까지 반영된 기록과 비교되므로 하강 방지가 배치 안에서도 작동한다. 같은 날 같은 단계 보도는 경과에 한 번만 남긴다 |
| 잠정합의 부결로 재교섭(S3)에 돌아가도 거부된 안의 타결 조건이 남았다 | 잠정합의 아래 단계로 내려가면 타결 조건을 지운다 |
| `사후조정서 합의 불발`이 조정 성립(S7)으로 읽혔다 | 성립·합의 뒤의 불발·실패·무산은 조정 불성립으로 읽는다 |
| `잠정합의 수순`, `잠정합의안 발표 예정`처럼 아직 합의가 없다는 제목이 S5로 반영됐다 | 잠정합의의 예고·전망 표현을 미래 가드로 잡아 교섭 단계로 유지한다 |
| `삼성전자서비스 노사, 임단협 체결`이 별칭 `삼성전자`에 걸려 삼성전자 기록을 덮으려 했다 | "법인명+노조" 근거도 별칭 뒤 단어 경계를 요구한다 |

원문 URL 되돌리기 예산도 설정의 법인 순서가 아니라 단계 신호가 있는 최신 기사부터 쓴다.

### 분류 성능 측정

규칙을 손대기 전후에 같은 입력으로 돌려 비교한다. 네트워크를 쓰지 않는다.

```bash
node scripts/measure-classification.mjs
```

저장된 후보(`public/data/news-candidates.json`)를 분류기에 다시 통과시켜 범위 판정 분포,
반영 가능 건수, 직영으로 판정됐지만 막힌 기사의 차단 사유를 출력한다. 2026-08-11 배치
306건 기준으로 개정 전후는 다음과 같다.
...
하청 격리 건수가 그대로인 것이 중요하다. 포함 범위를 넓히면서 배제 정밀도를 잃지 않았다는
뜻이다. 직영 판정 173건 중 하청 어휘가 남아 있는 기사는 0건이다.

남은 병목은 분류가 아니라 원문 URL 되돌리기 예산이다. 되돌리기 대상이 직영 판정 후보로
한정되어 있어 후보가 늘면 상한에 걸린다. `collectionPolicy.maxUrlResolutions`를 150으로
올렸고, 이 값을 더 올리면 실행 시간이 요청당 약 0.3초씩 늘어난다.

### schedule 이벤트 유실을 전제로 한다

GitHub의 schedule은 보장이 아니라 best-effort다. 부하가 몰리면 지연되고 유실되기도
한다. 2026-08-11 06:30 KST 예정분이 실제로 실행되지 않았고, cron 표기·기본 브랜치·
워크플로 활성 상태·Actions 권한·동시성·GitHub 장애 어디에도 원인이 없었다. 즉 이벤트
자체가 전달되지 않았다.

그래서 하루 한 번에 의존하지 않는다.

- 예비 스케줄을 KST 06:34·09:34·14:34 세 번 건다. 정시·30분 경계는 스케줄러 부하가
  몰리므로 피한다.
- 첫 단계인 **오늘 수집 필요 여부 판단**이 실행 흔적의 `lastRunKstDate`와
  `lastRunOutcome`를 보고, 오늘 이미 성공했으면 이후 단계를 모두 건너뛴다. 덕분에
  예비 실행이 중복 수집·커밋·배포 소음을 만들지 않는다. 앞선 시도가 실패했으면
  그대로 재시도가 된다.
- `수집 지연 감시` 워크플로(KST 13:23)가 `scripts/check-collection-freshness.mjs`로
  실행 흔적과 공개 페이지를 함께 확인하고, 멈춰 있으면 **실패**로 끝나 알림을 낸다.
  침묵을 성공으로 착각하지 않기 위한 장치다.
- 화면 상단이 마지막 수집 KST 날짜와 지연일수를 표시한다. 수집이 멈춘 날 화면이
  평소와 똑같이 보이면 안 된다.

남은 한계: 감시 워크플로도 GitHub schedule에 의존하므로 같은 유실 위험을 공유한다.
완전히 독립적인 감시가 필요하면 저장소 밖(예: Cloudflare Cron Trigger)에서
`workflow_dispatch`를 호출하는 경로를 추가해야 한다.

#### 한시적 예외: Cloudflare 이메일 인증 대기 (2026-08-16~)

Cloudflare 계정(`73907735ca1469d017e172fc9cca73c2`)은 2026-08-10에 프로젝트 시작 도구가
자동으로 만들어 정상 가입 절차를 거치지 않았고, 이메일이 미인증 상태다. wrangler는
워커를 다 올린 뒤 workers.dev 서브도메인 설정을 확인하는데 그 호출만 `code 10034`로
거부당한다. **배포 자체는 매번 성공하고 공개 화면도 최신이다.** 스텝만 실패로 끝난다.

대시보드에 재발송 버튼이 없고 같은 주소로 다시 입력하는 것도 거부되어, Cloudflare
지원으로 사람이 풀어야 한다. 그때까지 매일 빨간 실행이 쌓이면 진짜 배포 실패와
구분되지 않으므로, 배포 스텝이 **이 오류 하나만** 통과시킨다. 조건은 두 개를 함께 본다.

- 로그에 `10034`가 있다
- 로그에 `Uploaded er-radar`가 있다(업로드가 실제로 끝났다)

둘 중 하나라도 없으면 원래 종료 코드로 그대로 실패한다. 예외가 살아 있는 동안에는 매일
`::warning::` 한 줄이 실행 로그에 남는다.

**인증이 풀리면 할 일**: `.github/workflows/daily-bargaining-update.yml`의 배포 스텝에서
예외 블록을 지우고 `npx wrangler deploy` 한 줄로 되돌린다. 회귀 검증에 이 예외의 범위를
고정하는 테스트가 있으므로, 지울 때 그 테스트도 함께 지운다.

배포 단계는 Cloudflare Workers로 `npx wrangler deploy`를 실행한다. 화면이 읽는 사실
데이터와 실행 흔적은 빌드 시점에 번들로 들어가므로, 흔적을 기록한 뒤 다시 빌드해서
배포한다. 사실이 바뀌지 않은 날에도 배포하는 이유는 화면의 마지막 수집 시각을 최신으로
유지하기 위해서다. `CLOUDFLARE_API_TOKEN` 시크릿이 없으면 경고만 남기고 저장소
데이터만 갱신한다.
...
#### 잠정합의: 예고와 도출을 가른다 (2026-09-17)

`잠정합의 수순`, `잠정합의안 발표 예정`처럼 아직 합의가 없다는 뜻의 제목이 제목에
`잠정합의`가 들어 있다는 이유만으로 S5가 됐다. 2026-09-17 수집본에도
`SK하이닉스 노사, 밤샘 교섭 끝 임단협 잠정합의 수순`이 반영 가능 상태로 남아 있었다.
`tentative_agreement`의 미래 가드에 아래 형태를 추가해 `fallbackStage: bargaining` ·
`eventState: planned`로 보내고, 주 단계는 바꾸지 않는다.

| 가드가 잡는 형태 | 예 |
| --- | --- |
| 잠정합의 + 수순·초읽기·눈앞·임박·코앞·목전·가시화 | `임단협 잠정합의 수순` |
| 곧·조만간·이르면·머지않아 + 잠정합의 | `곧 잠정합의안 정리` |
| 잠정합의안 + 발표·공개·제시·도출·마련·정리·타결 + 예정·전망·앞두고 등 | `잠정합의안 발표 예정` |
| 잠정합의 + 여부·가능성·주목·촉각·분수령·기로 | `잠정합의 여부 주목` |
| 사측·회사·경영진이 잠정합의안을 제시·제안 | `사측, 잠정합의안 제시` |

가드의 간격 문자는 `[^.…·]`이라 문장 구분자를 넘지 않는다. 그래서 수순이 다른 말에
붙은 `카카오 노사, 임금협상 잠정 합의...‘봉합 수순’`은 그대로 S5로 남는다.
2026-09-17 후보 629건으로 재측정했을 때 이 변경으로 빠진 기사는 위 SK하이닉스
`수순` 제목 2건뿐이고, 실제 잠정합의 기사는 하나도 줄지 않았다.


#### 별칭 뒤 단어 경계 (2026-09-17)

"법인명 + 노조·노사" 근거는 별칭이 제목 어디에든 들어 있기만 하면 성립했다. 그래서
`삼성전자서비스 노사, 임단협 체결`이 삼성전자의 기사가 돼, 2026년 조인식(5월 27일) 기록을
8월 18일 자회사 기사로 덮어쓰려 했다. 교섭 용어 티어에만 있던 단어 경계 규칙을 이
티어에도 적용한다. 별칭 바로 뒤는 문장 끝·공백·문장부호·조사 한 글자여야 하고,
`포스코노조`처럼 노조·노동조합·지부·노사를 붙여 쓴 표기만 예외로 허용한다.
2026-09-17 후보 1,477건 중 20건이 제외됐고(삼성전자서비스·카카오뱅크·코레일네트웍스·
쿠팡CFS·쿠팡이츠·현대차그룹), 그 중 반영 가능 상태였던 3건은 모두 오분류였다.
...
다만 공개 시드는 법인당 한 교섭 테이블(교섭대표노조·공동교섭단)만 추적한다. 설정에
`publicRecordUnitIds`가 있는 법인은 단위 없는 제목을 그 테이블로 잇고
(`bargainingUnitScope: public_record_default`), 모든 노조에 일괄 반영하지 않는다.
제목에 사무직·조종사·소수노조 같은 말이 있으면 잇지 않는다. 제목이 공개 테이블이 아닌
단위를 가리키면 `tracksPublicRecordUnit: false`로 기록하고 반영하지 않는다.
...
처음 12곳은 모두 제조업이었다. 노사관계 노출도를 기준으로 다시 훑어 항공·금융·플랫폼·
철도·유통까지 14곳을 더해 26곳이 됐고, 화면 문구의 "주요 제조업"도 "주요 기업"으로
바꿨다. 이후 2021년까지 거슬러도 교섭 근거가 한 건도 나오지 않은 2곳을 빼 **24곳**이
됐다(아래 "추적 중단" 참고). 추가된 법인은 다음과 같다.
...
원문으로 확인한 사건만 시드에 넣었다. **확인하지 못한 연도는 비워 둔다.** 화면에서는
"원청 노조 교섭 근거 미확인"으로 남고, 일일 수집이 이후 채운다. 그래서 추적 법인 수와
연도별 레코드 수는 같지 않다 — 회귀 테스트도 두 값을 같다고 보지 않는다.

몇 가지 판단을 남긴다.

- **국민은행·신한은행의 2026년 교섭은 산별교섭이다.** 개별 기업이 아니라 금융산업
  노동조합과 사용자협의회가 교섭한다. 레코드는 지부 이름으로 남기고 주석에 적었다.
- **대한항공은 노조가 갈린다.** 일반직 노조는 2026-07-07 조인(S7), 조종사노조는 같은 해
  교섭이 결렬돼 쟁의 절차에 있다. 한 레코드에 섞지 않고 조인만 기록했다.
- **쿠팡은 쿠팡㈜ 직접고용 노조 교섭만 넣었다.** 쿠팡풀필먼트서비스는 별도 법인이라
  기존 제외 규칙을 그대로 적용했다.
- **LG디스플레이는 노조가 기사에 이름으로 나오지 않는다.** 임금 기사는 교섭 상대를 "노사"로만
  적어, 노동조합의 존재는 LG디스플레이 뉴스룸의 노동조합 태그에서 따로 확인하고 주석에 남겼다.

2026-08-25 조사분은 `data/historical-research-group-d.json`과
`data/current-2026-group-e.json`에 있다. **2026년 시드는 다시 빌드하지 않는다.** 빌드하면
그날 수집분이 지워지므로, 새 사실은 그룹 파일과 `data/current-2026-fact-seed.json`에 함께
넣는다.

### 추적 중단 (2026-08-25)

노조 홈페이지와 기사 두 경로로 2021년까지 거슬러 조회해도 **직접고용 노조 교섭 근거가 한
건도 나오지 않는 법인은 추적에서 뺀다.** 근거가 하나도 없는 법인을 남겨 두면 화면이
'미확인'으로만 채워지고, 그 미확인이 "아직 못 찾았다"인지 "애초에 교섭 노출이 없다"인지
구분되지 않는다. 지운 법인은 다음과 같다.

| 법인 | 조회 결과 |
| --- | --- |
| SK텔레콤 | 노조(sktunion.com)는 있으나 자료실이 조합원 로그인이고, 기사에서 확인되는 것은 2021년 성과급 갈등과 "올해 초 임단협 완료"라는 날짜 없는 언급뿐이다. |
| 포스코이앤씨 | 노사관계 기사는 건설노조의 원청 사용자성 판단뿐이다. 하청·간접고용 사건이라 기존 제외 규칙에 걸린다. |

삭제는 `data/company-universe.json`과 `data/source-config.json` **두 곳에서 함께** 해야
한다. 한쪽만 지우면 수집기가 매일 헛수집을 하거나 추적 법인이 수집에서 빠진다. 회귀
테스트가 두 파일의 법인 목록이 정확히 같은지, 그리고 추적 법인이 모두 2021년 이후 기록을
하나 이상 갖는지 본다. 이미 DB에 올라간 행은 `scripts/prune-supabase-orphans.mjs`로 정리한다.

### 경과가 교섭연도보다 먼저 시작하는 경우

쿠팡의 2026년 단체협약 교섭은 **2025년 10월 29일 상견례**로 시작해 이듬해 8월에 결렬됐다.
그런데 2026년 시드 검증기는 경과 날짜를 2026년으로만 받아, 이런 교섭은 시작점을 잃고
결렬 하루짜리 구간으로 그려졌다. `flowEvents`의 날짜는 이제 **2025~2026년**을 받는다.
대표 사건일(`eventDate`)은 그대로 교섭연도 안에 둔다.

한 가지 더 분명히 해 둔다. **일일 수집기는 기사 본문을 읽지 않는다.** 제목·매체·발행일·
URL만 남기는 것이 이 파이프라인의 원칙이라, 자동 수집은 기사 하나당 사건 하나밖에 만들지
못한다. "상견례 이후 17차례 교섭" 같은 본문 속 일정은 사람이 원문을 읽고 조사 레코드에
넣어야 한다. 화면의 구간이 한 칸뿐이라면 대개 근거 기사가 사건 하나만 담고 있어서지만,
쿠팡처럼 본문에 전체 일정이 있는데도 한 칸인 경우가 있으니 단일 구간 레코드는 주기적으로
원문을 다시 볼 값어치가 있다.

### 라틴 약칭과 한글 음차

기사 제목은 같은 회사를 "LG화학"과 "엘지화학", "SK하이닉스"와 "에스케이하이닉스"로 섞어
쓴다. 별칭 목록에 두 표기를 모두 적어 두는 것만으로는 적어 두지 않은 조합("㈜엘지디스플레이"
같은)을 계속 놓친다. 그래서 `collect-news.mjs`의 `comparableText`가 비교 직전에 음차를
약칭으로 접는다(엘지→lg, 에스케이→sk, 케이비→kb, 지엠→gm, 에이치디→hd). 제목과 별칭이
같은 함수를 지나므로 접는 방향은 한쪽만 정하면 된다.
...
- 공개 기록이 추적하는 교섭단위의 기사(`tracksPublicRecordUnit !== false`)
- 기존 기록보다 발생일이 늦고, 체결 보존·하강 방지 게이트를 통과함
- 법인마다 통과한 기사를 날짜순으로 차례로 적용한다(앞 기사까지 반영된 기록과 비교)
...
### DART 전자공시 수집 (2026-08-12 추가)

과거 교섭의 단계별 일정을 뉴스 검색으로 채우다 2021~2022년에서 막혔다. 오래된 기사는
색인에서 밀리고 같은 회사의 최근 교섭 기사가 대신 올라와서, 날짜를 특정할 수 없어 여러
건을 버렸다. 공시는 **접수일자가 규정으로 정해진 1차 근거**라 그 빈틈을 메운다.

```bash
OPENDART_API_KEY=... node scripts/collect-dart-disclosures.mjs --since 20210101
```

출력은 `data/dart-disclosure-candidates.json`이고, **후보 목록이지 공개 사실이 아니다.**
사람이 공시를 열어 확인한 뒤 `historical-flow-events.json`으로 옮긴다.

첫 실행에서 배운 것을 남긴다. 처음에는 `파업`·`쟁의`·`노동조합` 같은 말로 보고서명을
검색했고 **12개 법인 전부 0건**이었다. 공시 제목은 표준 서식명이라 사건을 그대로 쓰지
않는다. 실제로 잡아야 하는 것은 다음이다.

| 서식명 | 의미 |
| --- | --- |
| `생산중단` | 파업으로 인한 조업 정지가 이 서식으로 접수된다 |
| `풍문또는보도에대한해명` | 교섭·파업 보도에 대한 회사 입장 |
| `영업정지`·`직장폐쇄` | 사업장 폐쇄·정지 |

2021-01-01 이후 기준 후보 133건을 얻었다. 법인 12곳 모두 공시 고유번호가 매칭됐고
(비상장인 한국지엠과 사업회사 포스코도 이름으로 매칭된다), 포스코·한국지엠은 해당 기간
노사관계 공시가 없었다.

공시 본문은 저장하지 않는다. 회사명·보고서명·접수일자·접수번호와 열람 URL만 남긴다.
