...
- 접수 조건: 신청자 신원(회사·소속·직급·휴대전화·이메일), 근거 원문 URL(http·https), 수정 사유. 하나라도 없으면 접수하지 않는다.
...
### 접수 신원과 수동 변경 이력 (2026-08-26)

화면에서 **운영 관리 코드를 없앴다.** 코드 하나로 접수를 막으면 코드를 공유하는 순간 누가
넣었는지 알 수 없어지고, 코드를 모르는 현업은 오탈자 하나도 알릴 수 없다. 대신 신청자
신원(회사·소속·직급·휴대전화·이메일)을 필수로 받는다.

**신원은 인증이 아니다.** 그래서 접수만으로는 공개 데이터가 바뀌지 않는 순서를 그대로 뒀다.
관리자가 메일의 인증 링크에서 승인해야 반영된다. `DASHBOARD_ADMIN_CODE`는 더 이상 화면에
...
요청 대장은 "무엇을 요청했는지"를 담으므로 둘을 섞지 않는다. 섞으면 반려된 요청과 반영된
변경이 같은 줄에 남아 나중에 되짚을 수 없다.

적용할 마이그레이션:

```bash
supabase/migrations/20260826_requester_identity_and_audit.sql
```

필요한 시크릿은 다음과 같다. 없으면 접수는 되지만 즉시 메일이 나가지 않는다.

| 위치 | 이름 | 쓰임 |
| --- | --- | --- |
| Cloudflare | `GITHUB_DISPATCH_TOKEN` | 접수 즉시 알림 워크플로 호출 |
| GitHub | `DASHBOARD_ADMIN_CODE` | 인증 링크 서명 |
| GitHub | `MAIL_SMTP_USER` · `MAIL_SMTP_PASSWORD` | 메일 발송 |
